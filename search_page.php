...
<section class="scanner-form">

   <form action="" method="POST">
      <input type="number" class="box" name="scanner_input" placeholder="Scan product barcode..." required>
      <input type="submit" name="scanner_btn" value="Search" class="btn">
   </form>
...
<section class="products" style="padding-top: 0; min-height:100vh;">
   ...
   if(isset($_POST['scanner_btn'])){
      $scanner_input = $_POST['scanner_input'];
      $scanner_input = filter_var($scanner_input, FILTER_SANITIZE_NUMBER_INT);

      $select_product = $conn->prepare("SELECT * FROM `products` WHERE barcode = ?");
      $select_product->execute([$scanner_input]);
   ...
   <?php
         ...
         echo '<p class="empty">Product not found for the scanned barcode!</p>';
      }
   };
   ?>
